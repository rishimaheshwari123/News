            ...
            <tr key={news._id}>
              ...
              <td className="px-6 py-4 whitespace-nowrap">
                {news.images && news.images.length > 0 && (
                  <img className="h-10 w-10 rounded-full" src={news.images[0].url} alt={news.title} />
                )}
              </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{news.title}</td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{news.subtitle}</td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{news.category.name}</td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{news.subcategory.name}</td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{news.language}</td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{news.location}</td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{news.active ? 'Yes' : 'No'}</td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 space-x-2">
                <button 
                  onClick={() => handleToggleActive(news._id, news.active)}
                  className={`px-4 py-2 font-semibold text-sm ${news.active ? 'bg-red-500 text-white' : 'bg-green-500 text-white'} rounded-full shadow-sm`}
                >
