...
import { addCommentMain, addLikeMain, removeLikeMain } from '../services/operations/user';
...
function NewsDetails() {
  ...
  const handleComment = async () => {
    ...
    try {
      const response = await addCommentMain({ id: product._id, content: commentText }, token);
      setProduct(response);
      setCommentText('');
    } catch (error) {
        ...
        <div className="p-4 bg-gray-100 min-h-screen lg:w-[70%] w-full">
          {product?.category && product?.subcategory && (
            <div className="flex sm:flex-row items-center h-[20px] gap-2 mb-4 lg:w-[80%] w-full mx-auto flex-wrap">
              <h2 className="text-2xl font-bold text-gray-800">{product?.category.name}</h2>
              /
              <h2 className="text-sm text-gray-500">{product?.subcategory.name}</h2>
            </div>
          )}
          <div className="bg-white shadow-md rounded-lg p-6">
            <div className="w-full flex mb-6 justify-end items-center gap-1"><FaRegEye /> 50</div>
            <div className="lg:w-[80%] mx-auto">
              <h3 className="text-3xl font-semibold mb-2 text-gray-900 text-center">{product?.title}</h3>
              <h4 className="text-lg font-light mb-4 text-gray-700 text-center">{product?.subtitle}</h4>
            </div>
            <div>
            ...
            {product?.youtubeurl && (
              <div className="mb-6">
                <iframe
                  width="100%"
                  height="400"
                  src={`https://www.youtube.com/embed/${getYoutubeEmbedUrl(product?.youtubeurl)}`}
            ...
            <div className="mt-6">
              ...
              ></textarea>
              <button onClick={handleComment} className="mt-2 bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600 focus:outline-none">
                Comment
              </button>
            </div>
            <div className="mt-4">
                  ...
                  {product.comments
                    ...
                    .map((comment, index) => (
                      <li key={index} className="mb-2">
                        <span className="font-bold">{comment?.author.name}</span>: {comment?.content}
                      </li>
                    ))}
                </ul>
              ) : (
            ...
            {realted?.map((currElem, index) => (
              <Link to={`/newsdetails/${currElem._id}`} key={currElem._id}>
                <div className="flex gap-3">
                  <img src={currElem.images[0].url} alt="" className="w-[100px]" />
